...
- [`src/llm.rs`](/Users/azazo1/pjs/rust/outliner/src/llm.rs) 负责四类请求:
  - 基于页面文字的 TOC 定位判断
  - 基于页面图片的 TOC 定位, 目录结构提取, 目录提取和正文页码观测
  - 对批次结构结果的全局结构整理
- 目录定位时, 模型不会只回答"像不像目录", 而是必须对每个采样页返回 `hit`, `before`, `after`, `unknown` 之一
- 目录结构提取阶段, 模型先按批次提取层级线索, 再把所有批次的结构证据整理成一份最终层级图
- 目录提取阶段, 模型负责在最终层级图约束下, 从最终目录页里提取条目, 层级和印刷页码
...
- 有些目录页不会重复打印完整父级结构, 需要先从多批次证据中整理最终层级图
...
8. 渲染候选目录页, 让视觉 LLM 按批次提取目录结构线索
9. 把所有批次的结构结果做本地合并, 再交给 LLM 整理成最终层级图
10. 如果最终层级图不可靠, 自动回退到原始结构汇总结果
11. 让视觉 LLM 在最终层级图约束下提取目录条目, 层级和印刷页码
12. 对提取后的条目做本地归一化. 一方面压平跳级层级, 另一方面利用 `1.2`, `1.2.1`, `1.3` 这类编号关系做轻量修正, 必要时补一个 "目录" 顶级节点
13. 根据目录里出现的页码范围, 在正文区域再采样一些页面
14. 渲染这些正文页, 让视觉 LLM 读取页面上真实可见的印刷页码
15. 用观测到的页码样本推断页码偏移量, 把目录条目换算成 PDF 物理页
16. 读取 PDF 现有 outline, 做归一化对比
17. 如果已有 outline 已经一致, 则停止, 不写输出
18. 如果不一致, 使用 `qpdf` 写入新的 outline
19. 输出最终状态, 包括条目数, agent 调用次数和 token 用量
